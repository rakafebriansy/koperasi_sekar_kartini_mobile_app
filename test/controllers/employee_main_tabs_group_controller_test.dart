...
void main() {
  ...
  test("fetchListGroup mengambil daftar group dan mengisi state", () async {
    when(() => mockApi.fetchList<GroupModel>(
  ...
  test("fetchListVerifiedMember mengambil anggota aktif dan mengisi state",
      () async {
    ...
    expect(
        controller.verifiedMembers.first.name, DummyHelper.user(1).name);
  });

  test("fetchListInactiveMember mengambil anggota nonaktif dan mengisi state",
      () async {
    when(() => mockApi.fetchList<UserModel>(
          request: any(named: 'request'),
    ...
    expect(controller.inactiveMembers.first.name,
        DummyHelper.inuser(3).name);
  });
}
